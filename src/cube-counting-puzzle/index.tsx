...
export function CubeCountingPuzzle({ onHome }: Props = {}) {
  const [settings, setSettings] = useLocalStorage<Settings>('cubeCounting:settings', {
    count: 20,
    difficulty: 'mixed',
  });
          ...
          {onHome ? (
            ...
            >
              ...
              <div className="font-mono text-xs uppercase tracking-[0.2em] text-text-dim leading-tight text-left">
                <div className="group-hover:text-text">Block Counting</div>
...
function SetupScreen({
  ...
  const difficultyHint: Record<DifficultyOrMixed, string> = {
    easy: 'Small stacks, just a few hidden cubes.',
    normal: 'Taller stacks with more cubes tucked out of sight.',
    hard: 'Large, carved stacks with many hidden support cubes.',
  ...
  return (
    <div className="max-w-xl mx-auto rounded-2xl border border-border bg-bg-card p-6 md:p-8">
      <h2 className="font-display text-2xl font-semibold text-text mb-2">Block Counting</h2>
      <p className="text-text-dim text-sm mb-6 leading-relaxed">
        A stack of identical cubes is shown in 3D. Count how many cubes there are in total —
        including the hidden ones underneath that hold up what you can see. Answer the whole sheet,
        then submit to reveal everything at once.
      </p>
    ...
    <div className="rounded-xl border border-border bg-bg-card p-4 md:p-5">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
        ...
        <div className="font-mono text-xs text-text-dim">
          ...
          <span className="text-text">= {a.total} cubes</span>
        </div>
      </div>
      <div className="font-mono text-[11px] text-text-dim mb-3">
        <span className="text-text tabular-nums">{visible}</span> visible
        {' + '}
        <span className="text-text tabular-nums">{hidden}</span> hidden
        {' = '}
        <span className="text-accent tabular-nums">{a.total}</span> total
      </div>
        ...
        {puzzle.choices.map((c, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="tabular-nums mr-1">{c.value}</span>
              <span className="opacity-90">
                {c.isCorrect ? '— correct total' : `— ${c.rationale}`}
              </span>
            </div>
          );
        })}
  ...
  return (
    <svg width={28} height={28} viewBox="-12 -12 24 24" aria-hidden="true">
      <g stroke="var(--accent)" strokeWidth={1.5} fill="none" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M -9 -4 L 0 1 L 0 10 L -9 5 Z" opacity={0.6} />
        <path d="M 9 -4 L 0 1 L 0 10 L 9 5 Z" opacity={0.4} />
      </g>
    </svg>
  );
}

export default CubeCountingPuzzle;
