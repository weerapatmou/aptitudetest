...
function renderQuestion(_puzzle: Puzzle, svgs: string[], qIdx: number): string {
  ...
  const optSvgs = svgs.slice(1, 6); // up to 5 choices A–E

  const opts = optSvgs.map((s, i) => svgOptionBox(s, PRINT_LETTERS[i]!)).join('');

  return (
    `<div class="question">` +
    `<div class="q-label">Q${qIdx + 1} — Which outline is a rotation of the reference shape?</div>` +
...
export function exportFindingRotatedPdf(puzzles: Puzzle[]): void {
  ...
  const html = buildPageHtml(
    'Finding Rotated Shapes — Practice Sheet',
    containers.length,
    body + answerKey,
  );
