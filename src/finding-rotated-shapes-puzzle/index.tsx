...
import { exportFindingRotatedPdf } from './exportPdf';
          ...
          {onHome ? (
            ...
            >
              ...
              <div className="font-mono text-xs uppercase tracking-[0.2em] text-text-dim leading-tight text-left">
                <div className="group-hover:text-text">Finding Rotated Shapes</div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="font-mono text-xs uppercase tracking-[0.2em] text-text-dim leading-tight">
                <div>Finding Rotated Shapes</div>
...
function SetupScreen({
  ...
  return (
    <div className="max-w-xl mx-auto rounded-2xl border border-border bg-bg-card p-6 md:p-8">
      <h2 className="font-display text-2xl font-semibold text-text mb-2">Finding Rotated Shapes</h2>
      <p className="text-text-dim text-sm mb-6 leading-relaxed">
        A single outline shape is shown on the left. Pick the one choice (A–E) that is the same
        outline simply turned to a different angle. The rest are mirror images or have a corner
        changed, or are stretched or slanted. Answer the whole sheet, then submit to reveal
        everything at once. Every set is freshly generated.
      </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 items-start">
                <div className="relative rounded-2xl border-2 border-accent/40 bg-bg-card p-3 text-text shadow-[0_0_40px_-16px_var(--accent)]">
                  ...
                  <div className="w-full aspect-square flex items-center justify-center">
                    <ShapeFigure
                      shape={r.puzzle.reference}
                      viewBox={vb}
                      className="w-full h-full"
                      ariaLabel="Reference outline"
                    />
                  </div>
                </div>
      ...
      <div className="fixed bottom-0 inset-x-0 z-20 border-t border-border bg-bg/90 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => exportFindingRotatedPdf(results.map((r) => r.puzzle))}
          ...
          return (
            ...
            >
              ...
              <span className="opacity-90">
                {c.isCorrect
                  ? picked
                    ? '— the same outline, just turned ✓'
                    : '— the same outline, just turned (you missed it)'
                  : `— ${c.explanation}`}
              </span>
            </div>
          );
