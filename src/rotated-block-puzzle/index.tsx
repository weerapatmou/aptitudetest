...
import { exportRotatedBlockPdf } from './exportPdf';
...
export function RotatedBlockPuzzle({ onHome }: Props = {}) {
  const [settings, setSettings] = useLocalStorage<Settings>('rotatedBlock:settings', {
    count: 15,
    difficulty: 'mixed',
  });
  ...
  const selectChoice = useCallback(
    (qIdx: number, choiceIdx: number) => {
      ...
      setAnswers((prev) => ({
        ...prev,
        [qIdx]: prev[qIdx] === choiceIdx ? null : choiceIdx,
      }));
    },
    [submitted],
  );
  ...
  return (
    <div className="min-h-full bg-instrument">
      ...
      <BlockInspector
        puzzle={inspectIndex !== null ? (puzzles[inspectIndex] ?? null) : null}
          ...
          {onHome ? (
            ...
            >
              ...
              <div className="font-mono text-xs uppercase tracking-[0.2em] text-text-dim leading-tight text-left">
                <div className="group-hover:text-text">Rotated Blocks</div>
...
function SetupScreen({
  ...
  const difficultyHint: Record<DifficultyOrMixed, string> = {
    easy: 'Compact 4-block shapes — a simple turn apart.',
    normal: '5-block shapes with chiral twists.',
    hard: '6-block staircases with subtle mirror and moved-block traps.',
  ...
  return (
    <div className="max-w-xl mx-auto rounded-2xl border border-border bg-bg-card p-6 md:p-8">
      <h2 className="font-display text-2xl font-semibold text-text mb-2">Rotated Blocks</h2>
      <p className="text-text-dim text-sm mb-6 leading-relaxed">
        A 3D block is shown. Pick the one choice (A–E) that is the same block rotated to a different
        angle. The rest are mirror images or have a block changed. Answer the whole sheet, then
        submit to reveal everything at once.
      </p>
    ...
    <>
      ...
      <div className="flex flex-col gap-12 pb-28">
        {results.map((r, i) => {
          ...
          // One render3d frame shared by all cards in this question, so the
          // rotatable reveal views stay at a uniform scale.
          const vb3d = sharedViewBox3D([r.puzzle.reference, ...r.puzzle.choices.map((c) => c.solid)]);
          return (
            <section key={r.puzzle.id} id={`q-${i}`} data-pdf-q={i} className="flex flex-col gap-4 scroll-mt-24">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 items-start">
                <div className="relative rounded-2xl border-2 border-accent/40 bg-bg-card p-3 text-text shadow-[0_0_40px_-16px_var(--accent)]">
                  ...
                  {submitted ? (
                    <RotatableBlock
                      solid={r.puzzle.reference}
                      viewBox={vb3d}
                      className="w-full aspect-square flex items-center justify-center"
                      ariaLabel="Reference block"
                    />
                  ) : (
              ...
              {submitted && <RevealPanel result={r} onInspect={() => onInspect(i)} />}
            </section>
          );
        })}
      </div>
      ...
      <div className="fixed bottom-0 inset-x-0 z-20 border-t border-border bg-bg/90 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => exportRotatedBlockPdf(results.map((r) => r.puzzle))}
...
function RevealPanel({ result, onInspect }: { result: SheetResult; onInspect: () => void }) {
    ...
    <div className="rounded-xl border border-border bg-bg-card p-4 md:p-5">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={onInspect}
            className="inline-flex items-center gap-1.5 rounded-md border border-accent/40 bg-accent/5 px-2.5 py-1 font-mono text-[11px] uppercase tracking-wider text-accent hover:bg-accent/15 transition"
          >
            ⟳ Inspect in 3D
          </button>
          <div className="font-mono text-xs text-text-dim">
          ...
          return (
            ...
            >
              ...
              <span className="opacity-90">
                {c.isCorrect
                  ? picked
                    ? '— the same block, just rotated ✓'
                    : '— the same block, just rotated (you missed it)'
                  : `— ${c.explanation}`}
              </span>
            </div>
          );
